...
cargo test --lib               # 742 条，~1.2s
```

## 单元测试

742 条按模块分布（`cargo test --lib -- --list` 可复算）：
...
同日重测。上表 17 个工具合计 858 项断言（= 前 16 行那轮实测的 837 + `live_flow_control` 的
11；`live_consumer` 于 2026-09-24 因新增 C12/C12b 重测为 123，比原记的 113 多 10，合计随之 +10），`live_acl` 本机集群没开鉴权、`live_compression_matrix` 由脚本调度，
都不计进去）：
