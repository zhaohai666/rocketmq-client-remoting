...
cargo test --lib               # 777 条，~4s
```

## 单元测试

777 条按模块分布（`cargo test --lib -- --list` 可复算）：
...
2026-09-24 补的，同日实测；`live_subscribe` 是 2026-09-24 补的，同日实测。上表 20 个可计数
一行一行的数字相加 = **969 项断言**
...
`live_scheduled_intervals` 时按行重算才发现差 6），现在这个 969 是**按当前表格逐行重算**的结果
（961 + `live_subscribe` 8）。
