...
cargo test --lib               # 755 条，~1.2s
```

## 单元测试

755 条按模块分布（`cargo test --lib -- --list` 可复算）：
...
同日重测。上表 17 个工具合计 881 项断言（= 前 16 行那轮实测的 837 + `live_flow_control` 的
...
S5 启动期数值闸门从 11 涨到 25，多出的 14 项同样已计入），`live_acl` 本机集群没开鉴权、
