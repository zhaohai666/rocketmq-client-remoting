...
`live_scheduled_intervals` 时按行重算才发现差 6），现在这个 972 是**按当前表格逐行重算**的结果
（964 + `live_subscribe` 8）。
