...
cargo test --lib               # 760 条，~1.2s
```

## 单元测试

760 条按模块分布（`cargo test --lib -- --list` 可复算）：
...
同日重测；`live_fail_fast` 是 2026-09-24 补的，同日实测。上表 18 个工具合计 898 项断言（= 前 16 行那轮实测的 837 + `live_flow_control` 的
25；其中 `live_consumer` 于 2026-09-24 因新增 C12/C12b 重测为 123，比原记的 113 多 10，
`live_producer` 同日因新增 P11 重测为 71，比原记的 62 多 9，`live_flow_control` 同日因新增
S5 启动期数值闸门从 11 涨到 25，多出的 14 项同样已计入；`live_fail_fast` 的 17 项另计），`live_acl` 本机集群没开鉴权、
