...
dotnet test tests/RocketMQ.Client.Tests   # xunit，483 项测试
```
...
单测：`dotnet test tests/RocketMQ.Client.Tests` → **483 passed / 0 failed**，零 warning
...
- **异步发送背压的闸是手写公平信号量，语义与 Java 有三处不同。** `SendAsync` 在**调用方线程**
  ...
  （`free = total - 在途`，收缩可负）、`SetTotalPermits` 顺带叫醒等待者。② Java 的拒绝分支里
  「线程池满 + 开背压 ⇒ 就地跑完这一笔」在本端口没有对应物 —— 没有 `AsyncSenderExecutor`，
  每笔发送起一条后台线程，无队列可满。③ 只有「过闸」这一条支路有落点：
  Java 的另两条在 selector / pinned 异步入口上，本端口没有这两个入口。
